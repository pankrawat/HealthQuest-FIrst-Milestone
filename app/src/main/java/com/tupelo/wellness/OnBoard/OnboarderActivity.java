...
import com.tupelo.wellness.OnBoard.utils.ColorsArrayBuilder;
import com.tupelo.wellness.OnBoard.views.CircleIndicatorView;
...
import com.tupelo.wellness.view.CustomViewPager;
...
public abstract class OnboarderActivity extends AppCompatActivity implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private CustomViewPager vpOnboarderPager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vpOnboarderPager = (CustomViewPager) findViewById(R.id.vp_onboarder_pager);
        vpOnboarderPager.setContext(OnboarderActivity.this);
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.ib_next || i == R.id.fab && !isInLastPage) {
            ...
            if (vpOnboarderPager.isPagingEnabled()) {
                vpOnboarderPager.setCurrentItem(vpOnboarderPager.getCurrentItem() + 1);
            } else {
                Helper.mymoSerialCheck(flag,OnboarderActivity.this);
            }
    ...
    public void showNext(int request_flag) {
        if (flag == request_flag) {
            vpOnboarderPager.setPagingEnabled(true);
            vpOnboarderPager.setCurrentItem(vpOnboarderPager.getCurrentItem() + 1);
            DbAdapter dbAdapter = DbAdapter.getInstance(OnboarderActivity.this);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        Log.e("foooooooooooo", "the current position is " + position);

        buttonsLayout.setBackgroundColor(Color.parseColor(AppTheme.getInstance().colorPrimary));
    ...
    public void onPageSelected(int position) {
        ...
        Log.e("foooooooooooo", "the current position is 2 " + position);
        if (pages.get(position) != null) {
            int flag = pages.get(position).flag;
            this.flag = flag;
            if (flag == 1)
                vpOnboarderPager.setPagingEnabled(false);
        }

        vpOnboarderPager.setFlag(flag);
        ...
        if (this.shouldUseFloatingActionButton)
            this.fab.setImageResource(position == lastPagePosition ? R.mipmap.ic_done_white_24dp : R.mipmap.ic_arrow_forward_white_24dp);
